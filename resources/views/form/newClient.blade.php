                ...
                <form action="{{ route('client.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('adresse') is-invalid @enderror" id="adresse"
                            name="adresse" rows="3" required>{{ old('adresse') }}</input>
                        @error('adresse')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type_client') is-invalid @enderror" id="type_client"
                            ...
                            <option value="individuel" {{ old('type_client') == 'individuel' ? 'selected' : '' }}>Individuel
                            </option>
                            <option value="groupe" {{ old('type_client') == 'groupe' ? 'selected' : '' }}>Groupe</option>
                        </select>
                        @error('type_client')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
